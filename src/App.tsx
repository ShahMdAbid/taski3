...
import { Menu, X, Calendar as CalendarIcon, Settings, CheckSquare, BookOpen, MessageSquare, Wallet, Users as UsersIcon } from 'lucide-react';
...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'calendar' | 'settings' | 'academic' | 'expenses' | 'group-study'>('calendar');
  ...
  // Real-time Sync for Group Study Tasks
  useEffect(() => {
    ...
    const fetchGroupTasks = async () => {
      ...
      if (data) {
        setGroupTasks(data.map((t: any) => ({
          ...
          isHealth: t.is_health,
          isSpiritual: t.is_spiritual,
          isGroup: true,
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { 
        ...
        if (payload.eventType === 'INSERT') {
          ...
          const t = payload.new;
          setGroupTasks(prev => prev.map(old => old.id === t.id ? {
            id: t.id,
            title: t.title,
            date: t.date,
            time: t.time,
          ...
          setGroupTasks(prev => prev.filter(old => old.id !== payload.old.id));
        }
      })
      .subscribe();
  ...
  }, [currentView]); // Re-run when view changes to catch group changes
  ...
  const handleToggleTask = async (id: string) => {
    ...
    const isGrpTask = groupTasks.find(t => t.id === id);
    if (isGrpTask) {
      const { error } = await supabase
        .from('group_tasks')
        .update({ completed: !isGrpTask.completed })
        .eq('id', id);
  ...
  const handleDeleteTask = async (id: string) => {
    ...
    const isGrpTask = groupTasks.find(t => t.id === id);
    if (isGrpTask) {
      // Identity check: Can only delete if we are the author (optional but good)
      const savedMemberId = localStorage.getItem('study_member_id');
      if (isGrpTask.member_id && isGrpTask.member_id !== savedMemberId) {
      ...
      {isSidebarOpen && (
        <div className="w-64 bg-gray-900 dark:bg-gray-900 text-white flex flex-col h-full shrink-0 shadow-2xl z-20">
          ...
          <div className="p-3 flex-1 overflow-y-auto space-y-1">
            ...
            <button
              onClick={() => { setCurrentView('group-study'); setSelectedDate(null); setActiveNotebookId(null); setIsSidebarOpen(false); }}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-800 transition-colors text-sm font-medium text-left pl-6 border-l border-gray-800 ml-2"
            >
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden border-r border-gray-200 dark:border-gray-800">
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4 flex items-center justify-between shrink-0 transition-colors duration-200">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-gray-800 dark:text-white mr-2">
              {currentView === 'settings' ? 'Settings' :
                currentView === 'academic' ? (activeNotebookId ? 'Edit Goal' : 'Goal Plan') :
                  selectedDate ? 'Day Details' : 'Calendar'}
            </h1>
            <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl">
              <button
                onClick={() => { setCurrentView('calendar'); setSelectedDate(null); setActiveNotebookId(null); }}
                className={cn(
              ...
              >
                ...
                <span className="text-xs font-semibold hidden md:block">Calendar</span>
              </button>
              <button
                onClick={() => { setCurrentView('academic'); setSelectedDate(null); setActiveNotebookId(null); }}
                className={cn(
                  "p-2 rounded-lg transition-all duration-200 flex items-center gap-2",
              ...
              >
                ...
                <span className="text-xs font-semibold hidden md:block">Goals</span>
              </button>
            </div>
          </div>
