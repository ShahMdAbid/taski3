...
export default function DayView({ date, tasks, onToggleTask, onDeleteTask, setTasks, onBack }: DayViewProps) {
              ...
              <div
                ...
                className={cn(
                  ...
                  task.completed 
                    ...
                    : task.isGroup
                      ? "bg-indigo-50 dark:bg-indigo-900/10 border-indigo-200 dark:border-indigo-900 shadow-sm hover:shadow-md hover:border-indigo-300 dark:hover:border-indigo-700"
                      : task.isImportant
                  ...
                  <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => toggleTask(task.id)}>
                    ...
                    <div className={cn("flex flex-col", task.completed && "line-through text-gray-500 dark:text-gray-400")}>
                      <span className={cn(
                        ...
                        {task.isGroup && <span className="text-xs text-indigo-500 dark:text-indigo-400 mr-1.5 uppercase tracking-wider font-bold">[@ Group Job]</span>}
                        {task.title}
                      </span>
                      {task.time && (
      ...
      <div className="p-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 shrink-0 transition-colors duration-200">
        <form onSubmit={handleAddTask} className="flex gap-3 items-center">
          ...
          <div 
            ...
            className="relative w-32 flex items-center gap-2 bg-gray-50 dark:bg-gray-800 px-3 py-2.5 rounded-lg border border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 transition-all cursor-pointer group/time"
          >
