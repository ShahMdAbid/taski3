          ...
          <div className="mt-2 flex-1 overflow-y-auto space-y-1 custom-scrollbar">
            {dayTasks.filter(task => !task.isHealth && !task.isSpiritual).map(task => (
              <div 
                key={task.id} 
                className={cn(
                  "text-[10px] md:text-xs p-1 rounded truncate flex items-center gap-1.5 transition-colors border-l-2",
                  task.completed 
                    ? "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 line-through border-transparent" 
                    : task.isGroup
                      ? "bg-indigo-50 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300 border-indigo-500 font-medium"
                      : task.isImportant
                        ? "bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-800/60 font-medium border-transparent"
                        : "bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-800/60 border-transparent"
                )}
                onClick={(e) => {
                  e.stopPropagation();
              ...
              >
                {task.completed ? <CheckCircle2 className="w-3 h-3 shrink-0" /> : <Circle className="w-3 h-3 shrink-0 text-indigo-500/50" />}
                <span className="truncate">
                  {task.isGroup && <span className="opacity-70 mr-0.5 mt-[1px]">[@]</span>}
